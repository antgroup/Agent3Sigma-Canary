export function formatConsoleReport(report) {
  const lines = [
    ...
    `summary    critical=${report.summary.critical} high=${report.summary.high} medium=${report.summary.medium} low=${report.summary.low} info=${report.summary.info} auto_fixable=${report.summary.autoFixable}`,
...
export function formatSkillScanReport(report) {
  const lines = [
    'Clawkeeper Skill Scan',
    `skill_dir   ${report.skillDir}`,
    `score       ${report.score}/100`,
    `summary     critical=${report.summary.critical} high=${report.summary.high} medium=${report.summary.medium} low=${report.summary.low} info=${report.summary.info}`,
    ''
  ];
