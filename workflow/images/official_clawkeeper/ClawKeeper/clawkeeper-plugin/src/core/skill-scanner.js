...
async function loadRules(customRulesPath) {
  const rulesPath = customRulesPath
    ...
    : path.join(path.dirname(fileURLToPath(import.meta.url)), '..', 'config', 'core-rules.json');
...
function buildNextSteps(findings) {
  ...
  steps.push('Re-run `npx openclaw clawkeeper scan-skill <name-or-path>` after fixes.');
  return steps;
}
